    ...
    <el-table
      ...
      <el-table-column prop="icon" label="图标">
        <template #default="scope">
          <el-icon>
            <!-- 动态组件展示图标 -->
            <component v-if="scope.row.icon" :is="isValidTag(scope.row.icon) ? scope.row.icon : 'span'"></component>
          </el-icon>
        </template>
      </el-table-column>
...
const isValidTag = (tag: string) => {
  // 检查标签名称是否有效
  const validTags = ['span', 'div', 'el-icon', 'i']; // 添加你需要的有效标签
  return validTags.includes(tag) || /^[a-zA-Z][a-zA-Z0-9-]*$/.test(tag);
};
